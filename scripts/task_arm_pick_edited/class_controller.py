...
class ArmPickController(BaseController):
    ...
    def forward(
        ...
        # Convert 8-dim actions (7 arm + 1 gripper) to 9-dim joint commands (7 arm + 2 fingers)
        if actions is not None:
            arm_actions = actions[:7]  # First 7 are arm joint actions
            gripper_action = actions[7]  # Last one is binary gripper action
            # Duplicate gripper action for both fingers
            actions_9dim = np.concatenate([arm_actions, [gripper_action, gripper_action]])
        else:
            actions_9dim = None
            
        actions = ArticulationAction(joint_velocities=actions_9dim)
        # print(actions)
